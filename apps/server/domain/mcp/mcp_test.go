...
func TestApplyNoteMetadataDefaults(t *testing.T) {
	tests := []struct {
		name     string
		...
		{"no source", map[string]any{}, 0.7},
		{"preserves existing confidence", map[string]any{"source": "inferred", "confidence": 0.42}, 0.42},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			applyNoteMetadataDefaults(tt.props)

			conf, ok := tt.props["confidence"].(float64)
			if !ok {
				t.Fatalf("confidence missing or wrong type: %v (%T)", tt.props["confidence"], tt.props["confidence"])
			}
			if conf != tt.wantConf {
				t.Errorf("confidence = %v, want %v", conf, tt.wantConf)
			}
			if tt.props["tier"] != "archival" {
				t.Errorf("tier = %v, want %q", tt.props["tier"], "archival")
			}
			if uc, ok := tt.props["use_count"].(int); !ok || uc != 0 {
				t.Errorf("use_count = %v (%T), want int 0", tt.props["use_count"], tt.props["use_count"])
			}
			if nr, ok := tt.props["needs_review"].(bool); !ok || nr {
				t.Errorf("needs_review = %v (%T), want false", tt.props["needs_review"], tt.props["needs_review"])
			}
		})
	}
}
