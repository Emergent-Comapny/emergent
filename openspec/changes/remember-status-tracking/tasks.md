...
- [ ] 2.1 Add `ExecuteRememberStatus(ctx, projectID, args)` handler in `apps/server/domain/agents/mcp_tools.go`:
...
- [ ] 2.2 Register `remember-status` tool definition (name, description, input schema requiring `run_id`) alongside other agent-run tools
- [ ] 2.3 Add `remember-status` case to the MCP tool dispatch switch in `apps/server/domain/mcp/service.go` (`delegateAgentTool` or equivalent)
- [ ] 2.4 Add `remember-status` to `toolRequiredScope` mapping with the same scope as `agent-run-status`

## 3. REST route (optional parity)

- [ ] 3.1 Confirm whether a REST route is required for this change or MCP-only is sufficient for current callers; if required, add `GET /api/agents/runs/:runID/remember-status` handler reusing the same aggregation function
- [ ] 3.2 Add route registration + OpenAPI/swagger annotation if REST route is added

## 4. Remember/forget response wording

- [ ] 4.1 Update `remember`/`forget` MCP tool async-mode response text (`executeRemember`/`executeForget` in `apps/server/domain/mcp/service.go`) to reference `remember-status(run_id)` as the follow-up call
...
- [ ] 5.3 Update MCP README tool table/count to include `remember-status`
- [ ] 5.4 Run `go build ./...`, `task lint`, `task test` before marking complete
